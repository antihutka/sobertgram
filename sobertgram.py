...
def update_wrap(f):
  def wrapped(update: Update, context: CallbackContext):
    if not update.message:
      return nothing()
    is_blacklisted = \
      options.get_option(update.message.chat_id, 'blacklisted') > 0 or \
      options.get_option(update.message.from_user.id, 'user_blacklisted') > 0 or \
      (update.message.via_bot and options.get_option(update.message.via_bot.id, 'user_blacklisted') > 0)
    if is_blacklisted:
      blkLog.bad()
      return nothing()
    blkLog.good()
