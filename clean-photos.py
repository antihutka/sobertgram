...
def check_file_ids(cur):
  ...
  cnt = cur.fetchone()[0]
  cur.execute("SELECT file_ids.id FROM file_ids LEFT JOIN chat_files USING (file_id) LEFT JOIN chat_sticker USING (file_id) WHERE file_ids.id BETWEEN %s AND %s AND chat_files.id IS NULL AND chat_sticker.id IS NULL LIMIT 100000", (startid, endid))
  res = cur.fetchall()
...
def purge_duplicate_messages(cur):
  startid, endid, maxid = pick_startid(cur, "chat", rowcnt=4000000)
  cur.execute("SELECT COUNT(*) FROM chat WHERE id BETWEEN %s AND %s", (startid, endid))
  cnt = cur.fetchone()[0]
  cur.execute("SELECT id, hash FROM chat LEFT JOIN chat_hashcounts ON (hash=UNHEX(SHA2(text,256))) WHERE id BETWEEN %s AND %s AND convid IN (SELECT convid FROM options2 WHERE purge_chat>0) AND LENGTH(text) > 3 AND count > 1"
